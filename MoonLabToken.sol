...
contract MoonLabToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address public constant MKT_ADDRESS =
        0xbecc1378F65f033FDFFE409F42d873c801B09a00;
    ...
    uint256 public _maxTxAmount = _tTotal.mul(3).div(10**3);
    uint256 private minTokenNumberToSell = _tTotal.mul(3).div(10**3);
    ...
    constructor() {
        ...
        // Pancake router v2
        IPancakeRouter02 _pancakeSwapRouter =
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= minTokenNumberToSell;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
    ...
    function _tokenTransfer(
        ...
        // Calulate burn fee & marketing fee
        uint256 burnAmount = amount.mul(_burnFee).div(10**2);
        uint256 mktAmount = amount.mul(_mktFee).div(10**2);
        uint256 transferAmount = amount.sub(burnAmount).sub(mktAmount);
        require(
            amount == transferAmount + burnAmount + mktAmount,
            "Invalid transfer marketing and burn fee"
        );
        if (burnAmount > 0) _tokenTransfer(sender, BURN_ADDRESS, burnAmount);
